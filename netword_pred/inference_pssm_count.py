...
def network(inputs,seq_lens,max_seq_len,rnn_size,layers_size,keep_prob_ph):
    ...
    with tf.variable_scope('encoder_pssm_count'):
    ...
    with tf.variable_scope('fc_pssm_count'):
    ...
    with tf.variable_scope('fc1_pssm_count'):
...
def inference_pssm_count(input_data_pssm_count,seq_lens,max_seq_len,
                         rnn_size,layers_size,keep_prob_ph):

    with tf.name_scope('inference_pssm_count'):
        out = network(input_data_pssm_count,seq_lens,max_seq_len,rnn_size,layers_size,keep_prob_ph)

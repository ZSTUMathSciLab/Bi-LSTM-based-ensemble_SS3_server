...
def network(inputs,seq_lens,max_seq_len,rnn_size,layers_size,keep_prob_ph):
    ...
    with tf.variable_scope('encoder_pssm'):
    ...
    with tf.variable_scope('fc_pssm'):
    ...
    with tf.variable_scope('fc1_pssm'):
...
def inference_pssm(input_data_pssm,seq_lens,max_seq_len,rnn_size,layers_size,keep_prob_ph):

    with tf.name_scope('inference_pssm'):
        out = network(input_data_pssm,seq_lens,max_seq_len,rnn_size,layers_size,keep_prob_ph)
